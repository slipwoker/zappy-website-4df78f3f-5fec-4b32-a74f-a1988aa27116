document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getOrCreateErrorEl(field) {
      let errorEl = field.parentElement.querySelector('.' + ERROR_MESSAGE_CLASS);
      if (!errorEl) {
    ...
    function validateField(field) {
      ...
      if (isRequired && value === '') {
        showError(field, label.trim() + ' is required.');
        return false;
      }
      ...
      if (field.hasAttribute('minlength')) {
        ...
        if (value.length < min) {
          showError(field, label.trim() + ' must be at least ' + min + ' characters.');
          return false;
        }
      }
      ...
      if (field.hasAttribute('maxlength')) {
        ...
        if (value.length > max) {
          showError(field, label.trim() + ' must be no more than ' + max + ' characters.');
          return false;
        }
      }

      if (tagName === 'select' && isRequired && (value === '' || value === null)) {
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isValid) {
        ...
        if (firstInvalid) {
          ...
          const top =
            firstInvalid.getBoundingClientRect().top + window.pageYOffset - offset;
          window.scrollTo({ top: top, behavior: 'smooth' });
        }
        return;
      }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '4df78f3f-5fec-4b32-a74f-a1988aa27116',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '4df78f3f-5fec-4b32-a74f-a1988aa27116';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '4df78f3f-5fec-4b32-a74f-a1988aa27116';
